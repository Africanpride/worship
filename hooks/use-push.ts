...
export function usePush() {
						...
						if (Notification.permission === "granted" && payload.notification) {
							try {
								new Notification(payload.notification.title ?? "The NonStop", {
									...
									icon:
										(payload.notification.image as string) ?? "/logos/logo.png",
								});
							} catch {}
						}
					});
	...
	const subscribe = useCallback(async () => {
		if (!supported)
			throw new Error("Push not supported — use HTTPS or localhost");
		if (useFcm) {
			if (!fcmVapidKey)
				throw new Error(
					"FCM VAPID key not configured (NEXT_PUBLIC_FCM_VAPID_KEY or NEXT_PUBLIC_VAPID_PUBLIC_KEY)",
				);
			setLoading(true);
			try {
				...
				if (perm !== "granted")
					throw new Error(
						`Permission ${perm} — allow notifications in browser settings`,
					);
				if (!isFirebaseConfigured())
					throw new Error(
						"Firebase not configured (NEXT_PUBLIC_FIREBASE_* env)",
					);
				...
				if (!token)
					throw new Error(
						"FCM token empty — check Web Push certificate in Firebase Console > Cloud Messaging",
					);
				...
				if (!res.ok)
					throw new Error(
						(await res.json()).error ?? "Subscribe failed (server)",
					);
				setSubscribed(true);
		...
		if (!vapidPublicKey)
			throw new Error(
				"VAPID_PUBLIC_KEY not configured (restart dev server after .env change)",
			);
		setLoading(true);
		try {
			const perm = await Notification.requestPermission();
			...
			if (decoded.length !== 65)
				throw new Error(`Bad VAPID key (decoded ${decoded.length} != 65)`);
			if (!isSecureContext)
				throw new Error(
					"Not a secure context — push requires HTTPS or localhost",
				);
			...
			if (!rawKey || !authKey)
				throw new Error("Missing subscription keys (browser bug)");
	...
	return {
		...
		subscribe,
		unsubscribe,
		vapidConfigured: useFcm ? !!fcmVapidKey : !!vapidPublicKey,
		provider: useFcm ? "fcm" : "webpush",
	} as const;
}
