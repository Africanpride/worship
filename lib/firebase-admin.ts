...
function getServiceAccount(): { projectId: string; clientEmail: string; privateKey: string } | null {
  ...
  const jsonRaw = process.env.FIREBASE_SERVICE_ACCOUNT_JSON ?? process.env.GOOGLE_CREDENTIALS_JSON ?? "";
  if (jsonRaw) {
    try {
      const parsed = JSON.parse(jsonRaw) as { project_id?: string; projectId?: string; client_email?: string; clientEmail?: string; private_key?: string; privateKey?: string };
