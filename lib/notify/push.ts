...
export async function sendPushToUser(
	...
	if (fcmSubs.length > 0) {
		...
		if (!app) {
			log.warn(
				"system",
				"push skipped — Firebase Admin not configured (FIREBASE_SERVICE_ACCOUNT_JSON)",
				{ meta: { userId } },
			);
		} else {
					...
					try {
						...
						if (
							...
							await prisma.pushSubscription
								...
								.catch(() => {});
							log.info("system", "FCM token removed (invalid/unregistered)", {
								meta: { endpoint: sub.fcmToken?.slice(0, 24) },
							});
						} else {
							log.warn("system", "FCM send failed", {
								detail: msg,
								meta: { userId, code },
							});
						}
	...
	if (webSubs.length > 0) {
		...
		if (!vapid) {
			log.debug("system", "legacy web-push skipped — VAPID not configured", {
				meta: { userId },
			});
		} else {
					...
					try {
						...
						if (statusCode === 410 || statusCode === 404) {
							await prisma.pushSubscription
								.delete({ where: { id: sub.id } })
								.catch(() => {});
							log.info("system", "push subscription removed (410/404)", {
								meta: { endpoint: sub.endpoint?.slice(0, 32) },
							});
						} else {
