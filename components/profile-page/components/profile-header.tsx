...
export default function ProfileHeader({ user, profile }: ProfileHeaderProps) {
	...
	const upload = async () => {
		...
		try {
			...
			const patch = await fetch("/api/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ avatarUrl: url }) });
			if (!patch.ok) throw new Error("Save failed");
			toast.success("Profile picture updated");
			setOpen(false); setFile(null); setPreview(null);
			router.refresh();
		} catch (err) { toast.error(err instanceof Error ? err.message : String(err)); } finally { setUploading(false); }
	};
	const remove = async () => {
		setUploading(true);
		try {
			const r = await fetch("/api/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ avatarUrl: "" }) });
			if (!r.ok) throw new Error("Remove failed");
			toast.success("Profile picture removed");
			setOpen(false); setPreview(null); setFile(null);
			router.refresh();
		} catch (e) { toast.error(e instanceof Error ? e.message : String(e)); } finally { setUploading(false); }
	};
					...
					<Dialog open={open} onOpenChange={setOpen}>
						<div className="relative group shrink-0">
							...
							<DialogTrigger asChild>
								<Button size="icon" variant="secondary" className="absolute right-0 bottom-0 h-9 w-9 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" title="Change Profile Picture">
						...
						<DialogContent className="sm:max-w-md">
							<DialogHeader>
								...
								<DialogDescription>JPG/PNG/WebP, max 5MB. Stored on Cloudinary (l38psvpt) — 512×512 face crop.</DialogDescription>
							</DialogHeader>
								...
								<div className="flex justify-center">
									<div className="size-32 overflow-hidden rounded-full border bg-muted">
										{preview ? (
											...
											<div className="grid size-full place-items-center text-2xl font-bold text-muted-foreground">{initials}</div>
										)}
									</div>
								</div>
								<input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={onFileChange} />
								<div className="flex gap-2 justify-center">
									<Button type="button" variant="outline" onClick={() => inputRef.current?.click()} disabled={uploading} className="cursor-pointer">Choose image</Button>
									<Button type="button" onClick={upload} disabled={uploading || !file} className="cursor-pointer">{uploading ? <><Loader2 className="mr-2 size-4 animate-spin" /> Uploading…</> : "Upload"}</Button>
								</div>
								{avatarSrc && (
									<div className="flex justify-center">
										<Button type="button" variant="ghost" size="sm" onClick={remove} disabled={uploading} className="cursor-pointer text-destructive hover:text-destructive"><Trash2 className="mr-2 size-4" /> Remove photo</Button>
									</div>
								)}
							</div>
						</DialogContent>
